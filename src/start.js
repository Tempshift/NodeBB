...
function printStartupInfo() {
	if (nconf.get('isPrimary')) {
		...
		const host = nconf.get('postgres:host');
		const storeLocation = host ? `at ${host}${!host.includes('/') ? `:${nconf.get('postgres:port')}` : ''}` : '';

		winston.verbose('* using postgres store %s', storeLocation);
